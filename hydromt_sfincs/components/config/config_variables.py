...
class SfincsConfigVariables(BaseSettings):
    ...
    mmax: Optional[int] = Field(
        None, ge=1, description="Number of grid cells in x-direction",
        json_schema_extra={"always": True},
    )
    nmax: Optional[int] = Field(
        None, ge=1, description="Number of grid cells in y-direction",
        json_schema_extra={"always": True},
    )
    dx: Optional[float] = Field(
        None, gt=0, description="Grid size in x-direction",
        json_schema_extra={"always": True},
    )
    dy: Optional[float] = Field(
        None, gt=0, description="Grid size in y-direction",
        json_schema_extra={"always": True},
    )
    x0: Optional[float] = Field(
        None, description="Origin of the grid in the x-direction",
        json_schema_extra={"always": True},
    )
    y0: Optional[float] = Field(
        None, description="Origin of the grid in the y-direction",
        json_schema_extra={"always": True},
    )
    ...
    t1out: float = Field(
        -999.0, description="Output stop time (seconds)",
    )
    ...
    advection_scheme: str = Field(
        "upw1", description="Advection scheme ('upw1' or 'original')",
    )
    ...
    uvlim: float = Field(
        10.0, description="Velocity limiter (m/s)",
    )
    uvmax: float = Field(
        1000.0, description="Maximum velocity (m/s)",
    )
    ...
    wiggle_factor: float = Field(
        0.1, ge=0.0, description="Wiggle suppression factor (-)",
    )
    wiggle_threshold: float = Field(
        0.1, ge=0.0, description="Wiggle suppression minimum depth threshold (m)",
    )
    slopelim: float = Field(
        9999.9, ge=0.0, description="Slope limiter (-)",
    )
    btrelax: float = Field(
        3600.0, ge=0.0, description="Relaxation in uvmean (seconds)",
    )
    ...
    # ================================================================
    qinf: float = Field(
        0.0, ge=0.0, description="Infiltration rate, spatially uniform (mm/hr)",
    )
    qinf_zmin: float = Field(
        0.0, description="Minimum elevation for spatially uniform infiltration (m)",
    )
    sfacinf: float = Field(
        0.2, ge=0.0, le=1.0,
    ...
    horton_kr_kd: float = Field(
        10.0, description="Horton infiltration recovery vs decay ratio (-)",
    )
    ...
    freqminig: float = Field(
        0.0, ge=0.0, description="Minimum frequency wave maker IG spectrum (Hz)",
    )
    freqmaxig: float = Field(
        0.1, ge=0.0, description="Maximum frequency wave maker IG spectrum (Hz)",
    )
    ...
    wmfred: float = Field(
        0.99, description="Filtering variable in wave maker (-)",
    )
    wmsignal: str = Field(
        "spectrum", description="Wavemaker signal type ('spectrum' or 'mon')",
    )
    wmhmin: float = Field(
        0.1, description="Wavemaker minimum water depth (m)",
    )
    ...
    usespwprecip: int = Field(
        1, ge=0, le=1, description="Use rainfall from spiderweb (1: on, 0: off)",
    )
    spinup_meteo: int = Field(
        0, ge=0, le=1, description="Apply spinup to meteo forcing (1: on, 0: off)",
    )
    waveage: float = Field(
        -999.0, description="Determine Cd with wave age (-)",
    )
    factor_wind: float = Field(
        1.0, description="Wind forcing scale factor (-)",
    )
    factor_pres: float = Field(
        1.0, description="Pressure forcing scale factor (-)",
    )
    factor_prcp: float = Field(
        1.0, description="Precipitation forcing scale factor (-)",
    )
    factor_spw_size: float = Field(
        1.0, description="Spiderweb size scale factor (-)",
    )
    ...
    nh_fnudge: float = Field(
        0.9, description="Non-hydrostatic nudge factor (-)",
        json_schema_extra={"condition": "nonh == 1"},
    )
    nh_tstop: float = Field(
        -999.0, description="Non-hydrostatic stop time (seconds)",
        json_schema_extra={"condition": "nonh == 1"},
    )
    nh_tol: float = Field(
        0.001, description="Non-hydrostatic tolerance (-)",
        json_schema_extra={"condition": "nonh == 1"},
    )
    nh_itermax: int = Field(
        100, description="Non-hydrostatic maximum iterations (-)",
        json_schema_extra={"condition": "nonh == 1"},
    )
    ...
    structure_relax: float = Field(
        10.0, description="Structure relaxation factor (-)",
    )
    ...
    # ================================================================
    bathtub: int = Field(
        0, ge=0, le=1, description="Enable bathtub mode (1: on, 0: off)",
    )
    bathtub_fachs: float = Field(
        0.2, description="Bathtub wave height factor (-)",
        json_schema_extra={"condition": "bathtub == 1"},
    )
    bathtub_dt: float = Field(
        -999.0, description="Bathtub time step (seconds)",
        json_schema_extra={"condition": "bathtub == 1"},
    )
    ...
    inputformat: str = Field(
        "bin", description="Input file format (bin or asc)",
    )
    outputformat: str = Field(
        "net", description="Output file format (net or asc or bin)",
    )
    outputtype_map: Optional[str] = Field(
        None, description="Output format for map file (net or asc or bin)",
    )
    outputtype_his: Optional[str] = Field(
        None, description="Output format for his file (net or asc or bin)",
    )
    nc_deflate_level: int = Field(
        2, description="Netcdf deflate level (-)",
    )
    rugdepth: float = Field(
        0.05, description="Runup gauge depth threshold (m)",
    )
    h73table: int = Field(
        0, ge=0, le=1, description="Use h73 table (1: on, 0: off)",
    )
    ...
    ncinifile: Optional[str] = Field(None, description="Netcdf initial water level file")
    ...
    prcfile: Optional[str] = Field(None, description="Spatially uniform precipitation file")
    precipfile: Optional[str] = Field(None, description="LEGACY precipitation file (use prcfile)")
    ...
    infiltration_file: Optional[str] = Field(None, description="Infiltration file (alternative)")
    infiltration_type: Optional[str] = Field(None, description="Infiltration type")

    # ================================================================
    # Curve Number / Green-Ampt / Horton files
    # ================================================================
    scsfile: Optional[str] = Field(None, description="Curve Number max soil moisture file")
    smaxfile: Optional[str] = Field(None, description="Curve Number max storage file")
    sefffile: Optional[str] = Field(None, description="Curve Number initial storage file")
    psifile: Optional[str] = Field(None, description="Green-Ampt suction head file")
    sigmafile: Optional[str] = Field(None, description="Green-Ampt max moisture deficit file")
    ksfile: Optional[str] = Field(None, description="Green-Ampt hydraulic conductivity file")
    f0file: Optional[str] = Field(None, description="Horton max infiltration capacity file")
    ...
    # Netcdf input files
    # ================================================================
    netbndbzsbzifile: Optional[str] = Field(None, description="Netcdf water level input file")
    netsrcdisfile: Optional[str] = Field(None, description="Netcdf discharge input file")
    netamuamvfile: Optional[str] = Field(None, description="Netcdf wind input file")
    netamprfile: Optional[str] = Field(None, description="Netcdf precipitation input file")
    netampfile: Optional[str] = Field(None, description="Netcdf atmospheric pressure input file")
    ...
    storeqdrain: int = Field(1, ge=0, le=1, description="Write drainage discharge output (1: yes, 0: no)")
    storezvolume: int = Field(0, ge=0, le=1, description="Write storage volume output (1: yes, 0: no)")
    storestoragevolume: int = Field(0, ge=0, le=1, description="Write total storage volume output (1: yes, 0: no)")
    store_tsunami_arrival_time: int = Field(0, ge=0, le=1, description="Write tsunami arrival time output (1: yes, 0: no)")
    store_dynamic_bed_level: int = Field(0, ge=0, le=1, description="Write dynamic bed level output (1: yes, 0: no)")
    regular_output_on_mesh: int = Field(0, ge=0, le=1, description="Write regular grid on quadtree mesh output (1: yes, 0: no)")
    twet_threshold: float = Field(0.01, ge=0.0, description="Time-wet minimum depth threshold (m)")
    tsunami_arrival_threshold: float = Field(0.01, ge=0.0, description="Tsunami arrival minimum depth threshold (m)")
    timestep_analysis: int = Field(0, ge=0, le=1, description="Enable timestep analysis output (1: yes, 0: no)")
    debug: int = Field(0, ge=0, le=1, description="Debug mode — write every timestep (1: yes, 0: no)")
    percentage_done: int = Field(5, ge=1, description="Percentage done output interval (%)")
