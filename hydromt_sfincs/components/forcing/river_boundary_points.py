...
class SfincsRiverBoundaryPoints(ModelComponent):
    ...
    def create( 
        ...
        **kwargs
        ...
        if not (gdf_out_pts.geom_type == "Point").all():
            raise ValueError("gdf_out_pts must contain Point geometries.")

        gdf_riv = self.data_catalog.get_geodataframe(
            centerlines, geom=self.model.region, **kwargs
        ).to_crs(self.model.crs)

        if not (gdf_riv.geom_type == "LineString").all():
            raise ValueError("gdf_riv must contain LineString geometries.")

        gdf_lines = gdf_riv[["geometry"]].copy().reset_index(drop=True)
        sidx = gdf_lines.sindex
